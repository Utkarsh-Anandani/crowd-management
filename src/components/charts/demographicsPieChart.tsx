...
export function DemographicsPieChart({ selectedDate, selectedSite }: PieChartProps) {
    ...
    const fetchDemographicsData = async () => {
      ...
      if (data && data.buckets) {
        ...
        setChartData([
          ...
          { gender: "Female", count: totalFemale, fill: chartConfig.female.color },
        ])
      }
    }
  ...
  return (
    <div className="bg-white rounded-lg py-4 flex flex-col w-full lg:items-start items-center lg:w-1/3">
      <h3 className="font-medium text-[16px] ml-4 text-[#1D1D1B]">Demographics Analysis</h3>
      <ChartContainer config={chartConfig} className="aspect-square h-50 w-50">
        <PieChart>
          <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel />} />
          <Pie data={chartData} dataKey="count" nameKey="gender" innerRadius={60} strokeWidth={5}>
            <Label
              content={({ viewBox }) => {
      ...
      <div className="flex flex-col gap-1 ml-4">
        <div className="flex flex-row items-center gap-3">
          ...
          <p className="font-normal text-[16px] text-[#030303]">
            <span className="font-medium">{Math.round((chartData[0].count / totalVisitors) * 100)}%</span> Males
          </p>
        </div>
        <div className="flex flex-row items-center gap-3">
          <img src="/dashboard/female.svg" alt="female" />
          <p className="font-normal text-[16px] text-[#030303]">
            <span className="font-medium">{Math.round((chartData[1].count / totalVisitors) * 100)}%</span> Females
          </p>
        </div>
      </div>
    </div>
